...
#'
#'@return (Note for Mark (RANDOM TEXT): your original text was, I think, too brief. Can we list all the objects that are returned by hettreatreg here, like in the official R help for "lm," for example? If you write the first draft of this list, I will then update/edit it.)
#'
#' \describe{
...
hettreatreg <- function(outcome, treatment, covariates) {
  ...
  # ---- Calculate Treatment weights and OLS2
  ...
  # display results:
  cat('"OLS" is the estimated regression coefficient on', treatment_name, '\n', '\n')
  cat('   OLS =', signif(beta_treated, 4), '\n', '\n')
  cat('P(d=1) =', round(p1, 3), '\n')
  cat('P(d=0) =', round(p0, 3), '\n', '\n')
  cat('    w1 =', round(w1, 3), '\n')
  cat('    w0 =', round(w0, 3), '\n')
  cat(' delta =', round(delta, 3), '\n', '\n')
  cat('   ATE =', signif(ate, 4), '\n')
  cat('   ATT =', signif(att, 4), '\n')
  cat('   ATU =', signif(atu, 4), '\n', '\n')
  cat('OLS = w1*ATT + w0*ATU =', signif(ols2, 4), '\n')

  # save results in a list:
  invisible(list(OLS1 = beta_treated,
              'P(d=1)' = p1,
              'P(d=0)' = p0,
              w1 = w1,
              w0 = w0,
